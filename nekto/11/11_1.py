...
def apply_width_expansion(galaxy: [], columns: set()):
    galaxy[0] = galaxy[0] + sum(1 for column in columns if column < galaxy[0])
    return galaxy
...
for galaxy in galaxies_coordinates:
    galaxies_coordinates_with_width_expansion.append(
        apply_width_expansion(galaxy, columns_witout_galaxies)
    )
